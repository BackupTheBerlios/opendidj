...
#ifdef CPU_LF1000 /* use exact mappings */
	switch(layer) {
		case 0:
		ctl += MLCCONTROL0;
		break;
...
#else /* registers are evenly spaced */
	ctl += MLCCONTROL0+0x20*layer;
#endif
...
#ifdef CPU_LF1000 /* registers are not evenly spaced */
	switch(layer) {
		case 0:
...
#else	/* registers are evenly spaced */
	reg = mlc.mem+MLCADDRESS0+layer*0x20;
#endif
	iowrite32(addr, reg);
	return 0;
}
...
#else
	iowrite32(hstride,mlc.mem+MLCHSTRIDE0+layer*0x20);
#endif
	return 0;
}
...
#else
	return ioread32(mlc.mem+MLCHSTRIDE0+layer*0x20);
#endif
}
...
#else
	reg = mlc.mem+MLCVSTRIDE0+layer*0x20;
#endif
...
#else /* CPU_MF2530F */
		iowrite32(vstride, mlc.mem+MLCVSTRIDECB);
		iowrite32(vstride, mlc.mem+MLCVSTRIDECR);
#endif
	}
	return 0;
}
...
int mlc_SetTransparencyAlpha(u8 layer, u8 alpha)
{
	...
	reg = mlc.mem+MLCTPCOLOR0+layer*0x20;
...
int mlc_GetTransparencyAlpha(u8 layer)
{
	...
	tmp = ioread32(mlc.mem+MLCTPCOLOR0+layer*0x20);
...
int mlc_SetInvertColor(u8 layer, u32 color)
{
	...
	reg = mlc.mem+MLCINVCOLOR0+layer*0x20;
...
int mlc_GetInvertColor(u8 layer, int *color)
{
	...
	tmp = ioread32(mlc.mem+MLCINVCOLOR0+layer*0x20);
...
int mlc_Set3DEnable(u8 layer, u8 en)
{
	...
	if(en) {
	    int l;
		/* ensure that no other layer has 3DENB set */
		for(l = 0; l < MLC_NUM_LAYERS; l++) {
			if(l != layer) {
				reg = SelectLayerControl(l);
				tmp = ioread32(reg);
				if(IS_SET(tmp,GRP3DENB)) {
					BIT_CLR(tmp,GRP3DENB);
					iowrite32(tmp, reg);
				}
			}
		}
...
int mlc_SetFormat(u8 layer, enum RGBFMT format)
{
	...
	if(layer > MLC_NUM_LAYERS || layer == MLC_VIDEO_LAYER || 
		...
		return -EINVAL;
#else
	if (layer > MLC_NUM_LAYERS || format > 0xFFFF)
		return -EINVAL;
	if (layer == MLC_VIDEO_LAYER) /* video layer */
		format &= 0x0003; /* video format != RGB format */
#endif
...
#else
	iowrite32(((top<<TOP)|(left<<LEFT)),
				mlc.mem+MLCLEFTOP0+0x20*layer);
	iowrite32(((right<<RIGHT)|(bottom<<BOTTOM)),
				mlc.mem+MLCRIGHTBOTTOM0+0x20*layer);
#endif
	return 0;
}
...
#else
	tmp = ioread32(mlc.mem+MLCLEFTOP0+0x20*layer);
	p->top  = ((tmp & (0x7FF<<TOP))>>TOP);
	p->left = ((tmp & (0x7FF<<LEFT))>>LEFT);

	tmp = ioread32(mlc.mem+MLCRIGHTBOTTOM0+0x20*layer);
	p->right  = ((tmp & (0x7FF<<RIGHT))>>RIGHT);
	p->bottom = ((tmp & (0x7FF<<BOTTOM))>>BOTTOM);
#endif
	return 0;
...
int mlc_GetLayerEnable(u8 layer, int *en) /* FIXME */
{
	u32 tmp;
	void *reg;
