...
static int lf1000_dma_probe(struct platform_device *pdev)
{
	...
	if(!res) {
		printk(KERN_ERR "dma: failed to get resource\n");
		return -ENXIO;
	}

	if(!request_mem_region(res->start, (res->end - res->start)+1,
				"lf1000_dma")) {
		printk(KERN_ERR "dma: failed to map DMA region\n");
		return -EBUSY;
	}
	...
	if(dev.iomem == NULL) {
		printk(KERN_ERR "dma: failed to ioremap\n");
		ret = -ENOMEM;
		goto fail_remap;
	}
	...
	if(ret) {
		printk(KERN_ERR "dma: Wow!  Can't register IRQ\n");
		goto fail_irq;
	}
...
struct platform_driver lf1000_dma_drv = {
