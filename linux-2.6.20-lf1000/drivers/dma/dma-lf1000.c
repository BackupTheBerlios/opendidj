...
/* internal functions */
//static int __devinit lf1000_dma_probe(struct pci_dev *pdev, const struct pci_device_id *ent);
	...
	gfp_t flags)
{
	...
	desc = dma_pool_alloc(ioat_device->dma_pool, flags, &phys);
	if (unlikely(!desc))
		return NULL;

	desc_sw = kzalloc(sizeof(*desc_sw), flags);
	if (unlikely(!desc_sw)) {
		dma_pool_free(ioat_device->dma_pool, desc, phys);
		return NULL;
	}
...
static int ioat_dma_alloc_chan_resources(struct dma_chan *chan)
{
	...
	ioat_chan->completion_virt =
		dma_pool_alloc(ioat_chan->device->completion_pool,
		               GFP_KERNEL,
...
static void ioat_dma_free_chan_resources(struct dma_chan *chan)
{
	...
	list_for_each_entry_safe(desc, _desc, &ioat_chan->used_desc, node) {
		in_use_descs++;
		list_del(&desc->node);
		dma_pool_free(ioat_device->dma_pool, desc->hw, desc->phys);
		kfree(desc);
	}
	...
	dma_pool_free(ioat_device->completion_pool,
	              ioat_chan->completion_virt,
	              ioat_chan->completion_addr);
...
#if	0
static struct pci_driver lf1000_dma_drv = {
	.name 	= "lf1000_dma",
...
static irqreturn_t lf1000_do_interrupt(int irq, void *data)
{
...
#if	1
//static int __devinit lf1000_dma_probe(struct pci_dev *pdev,
//                                const struct pci_device_id *ent)
static int __devinit lf1000_dma_probe(struct platform_device *pdev)
{
	int err;
//	unsigned long mmio_start, mmio_len;
	void __iomem *reg_base = IO_ADDRESS( 0xc0000000);
	...
	err = pci_request_regions(pdev, lf1000_dma_drv.name);
	if (err)
	...
	device->dma_pool = dma_pool_create("dma_desc_pool", pdev,
	...
	device->completion_pool = dma_pool_create("completion_pool", pdev, sizeof(u64), SMP_CACHE_BYTES, SMP_CACHE_BYTES);
...
//		device);
	err = request_irq( 3, &lf1000_do_interrupt, IRQF_SHARED, "lf1000_dma",
		device);
	if (err)
		goto err_irq;
...
err_self_test:
err_irq:
	dma_pool_destroy(device->completion_pool);
err_completion_pool:
...
#if	0
static int __devinit lf1000_dma_probe(struct platform_device *dev)
{
