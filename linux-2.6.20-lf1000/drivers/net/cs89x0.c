...
#elif defined CONFIG_MACH_LF_MP2530F
	/* use nCS1 for chip select */
	gpio_configure_pin(GPIO_PORT_D, 0, GPIO_ALT1, 1, 0, 0);
	/* take the chip out of reset */
	gpio_configure_pin(GPIO_PORT_B, 30, GPIO_GPIOFN, 1, 0, 0);
#endif /* CONFIG_MACH_LF_MP2530F */
...
net_close(struct net_device *dev)
{
	...
	free_irq(dev->irq, dev);
#else
	#ifdef CONFIG_MACH_ME_LF1000 /* on a GPIO pin */
	gpio_free_irq(ETHER_GPIO_PORT, ETHER_GPIO_PIN, 
			(gpio_irq_handler_t)net_interrupt);
	#else /* on an EIRQ pin */
	free_irq(LF1000_irq, dev);
	#endif
#endif
