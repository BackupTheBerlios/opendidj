...
static struct device_attribute vol_mtd_name =
	__ATTR(vol_mtd_name, S_IRUGO, vol_attribute_show, NULL);
static struct device_attribute vol_mtd_num =
...
#ifdef CONFIG_MTD_UBI_GLUEBI
	else if (attr == &vol_mtd_name)
		ret = sprintf(buf, "%s\n", vol->gluebi_mtd.name);
	else if (attr == &vol_mtd_num)
...
static int volume_sysfs_init(struct ubi_device *ubi, struct ubi_volume *vol)
{
	...
	if (err)
		return err;
#ifdef CONFIG_MTD_UBI_GLUEBI
	err = device_create_file(&vol->dev, &vol_mtd_name);
	if (err)
		return err;
	err = device_create_file(&vol->dev, &vol_mtd_num);
	if (err)
		return err;
#endif
	return 0;
...
{
#ifdef CONFIG_MTD_UBI_GLUEBI
	device_remove_file(&vol->dev, &vol_mtd_num);
	device_remove_file(&vol->dev, &vol_mtd_name);
#endif
