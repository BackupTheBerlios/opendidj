...
#define UBI_MAJOR 240
#define NUM_UBI_MAJORS 4
...
static struct device_attribute dev_mtd_dev_name =
	__ATTR(mtd_dev_name, S_IRUGO, dev_attribute_show, NULL);
static struct device_attribute dev_mtd_dev_num =
	__ATTR(mtd_dev_num, S_IRUGO, dev_attribute_show, NULL);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (attr == &dev_eraseblock_size)
		...
		return sprintf(buf, "%d\n", ubi->thread_enabled);
	else if (attr == &dev_mtd_dev_name)
		return sprintf(buf, "%s\n", ubi->mtd->name);
	else if (attr == &dev_mtd_dev_num)
		return sprintf(buf, "%d\n", ubi->mtd->index);
	else
		BUG();
...
static int ubi_sysfs_init(struct ubi_device *ubi)
{
	...
	err = device_create_file(&ubi->dev, &dev_mtd_dev_name);
	if (err)
		goto out_bgt_enabled;
	err = device_create_file(&ubi->dev, &dev_mtd_dev_num);
	if (err)
		goto out_mtd_dev_name;

	return 0;

out_mtd_dev_name:
	device_remove_file(&ubi->dev, &dev_mtd_dev_name);
out_bgt_enabled:
...
{
	device_remove_file(&ubi->dev, &dev_mtd_dev_num);
	device_remove_file(&ubi->dev, &dev_mtd_dev_name);
	device_remove_file(&ubi->dev, &dev_bgt_enabled);
	device_remove_file(&ubi->dev, &dev_min_io_size);
	device_remove_file(&ubi->dev, &dev_max_vol_count);
...
static int uif_init(struct ubi_device *ubi)
{
	...
	/* BC: We made the major number static instead of dynamic because all of
	 * our custom devices are static and we're getting some overlap.
	 */
	/* The scheme is a new major for each UBI device, and the minors
	 * correspond to the volumes.  Drat.  This must be fixed!  For now we
	 * hack it.
	 */
	if(ubi_devices_cnt == NUM_UBI_MAJORS) {
		ubi_err("Sorry.  Not enough majors for your ubi layout.\n");
		return -EINVAL;
	}
	dev = MKDEV(UBI_MAJOR + ubi_devices_cnt, 0);
