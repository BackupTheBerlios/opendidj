/*
 *	Real Time Clock interface for Linux on Magic Eyes LF1000
 *
 *	Copyright (C) 2007 Leapfrog
 *      Scott Esters
 *
 ...
 */
static int lf1000_rtc_probe(struct platform_device *pdev)
{
	struct resource *res;
 ...
 */
static int lf1000_rtc_remove(struct platform_device *pdev)
{
	struct rtc_device *rtc = platform_get_drvdata(pdev);
	struct resource *res;
...
static struct platform_driver lf1000_rtc_driver = {
	.probe		= lf1000_rtc_probe,
	.remove		= lf1000_rtc_remove,
...
{
	return platform_driver_register(&lf1000_rtc_driver);
}
