...
if [ "$BRIO" == "1" ]; then
	$PROJECT_PATH/scripts/brio_install.sh $*
fi
...
if [ "$EMBEDDED" -eq "1" ]; then
	...
	mkfs.jffs2 -l -n -e 128KiB -p -o $ROOTFS_PATH/../erootfs-tmp.jffs2 -d $ROOTFS_PATH
	sumtool -l -n -e 128KiB -p -i $ROOTFS_PATH/../erootfs-tmp.jffs2 -o $ROOTFS_PATH/../erootfs.jffs2
