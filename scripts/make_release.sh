...
# We always release erootfs and nfsroot now
ROOTFS_RELEASE=$VERSION-$TARGET_MACH
ROOTFS_PATH=$LINUX_DIST_DIR/nfsroot-$ROOTFS_RELEASE $PROJECT_PATH/scripts/make_rootfs.sh $*
...
cp -f $LINUX_DIST_DIR/nfsroot*-LF_LF1000/usr/bin/mkbase.sh $LINUX_DIST_DIR/mfg-cart/ATAP
...
cp -f $LINUX_DIST_DIR/nfsroot*-LF_LF1000/usr/bin/mkbase.sh $LINUX_DIST_DIR/mfg-cart/Base2ATAP
CUR_PATH="`pwd`"
